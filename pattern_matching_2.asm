.code:
    ...
    NOTEQL:
        ...
        JZ NOT_FOUND    ; if no character is left in main string then obviously the substring doesn't exists in main string
        JMP FIND
    ...
    CHECK:
        ...
        MOV SP, SI      ; storing the index of current character of main string so if the following REPE CMPSB find mismatch,
                        ; then the process can be started over from the next character of main string (SEE line 1 of TEMP_NOT_FOUND) by going to TEMP_NOT_FOUND > FIND
        ADD SP, 0001H
        CLD
        REPE CMPSB
        JNE TEMP_NOT_FOUND
    ...
    TEMP_NOT_FOUND:     ; substring not found starting from the current character of main string,
...
.data:
    STR1 DB 'ashutosh'       ; Getting input for the string
    LEN1 DW ($-STR1)        ; Storing the length of STR1
    STR2 DB 'tosh'           ; Getting input for the search pattern
